...
/*------------------------------------------------------------
	...
	Description	: Staff-tshirt web site send order mail class
	Hash		: Session data
...
//						$printinfo .= "◇デザインサイズ：　".$sizeName[$a2[$i]['areasize']]."\n";
	...
	*/
	protected function send_mail($mail_text, $name, $to, $attach){
		...
		$suffix = "【Staff-Tshirt】"; 				// 件名の後ろに付加するテキスト
		...
		if(mail($sendto, $subject, $msg, $header)){
			...
			$msg = $name."　様\n";
			$msg .= "このたびは、スタッフTシャツをご利用いただき誠にありがとうございます。\n";
			$msg .= "お申し込みを承りました。\n";
