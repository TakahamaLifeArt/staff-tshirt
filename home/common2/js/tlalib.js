...
*	log	
*	2016-10-25	staff-tshirt用に一部機能を廃止
*/
...
$.getScript('/common2/js/phonedata.js');
...
****************************************************************************************************************************/
		...
		*	2016-10-25 廃止
		$(window).on('keypress', 'form', 'input:not([class^=for])', function(e){
		...
		}).on('focusout', 'form', 'input:not([class^=for])', function(e){
		...
		*/
		...
		$(window).on('keypress', '.forNum', function(e){
			$(this).restrictKey(e, 'num');
		}).on('focusout', '.forNum', function(e){
			$.check_NaN(this);
		});
		
		// 0と自然数　0から9 のみ入力、桁区切りなし、不正値は""
		$(window).on('keypress', '.forBlank', function(e){
			$(this).restrictKey(e, 'num');
		}).on('focusout', '.forBlank', function(e){
			$.check_NaN(this,"");
		});
		
		// 0から9 . - のみ入力、桁区切りなし、不正値は"0"
		$(window).on('keypress', '.forReal', function(e){
			$(this).restrictKey(e, 'price');
	    }).on('focusout', '.forReal', function(e){
	    	$.check_Real(this);
		});
		
		// 金額　0から9 . - のみ入力、桁区切りあり、フォーカスでカンマなしに変換、不正値は"0"
		$(window).on('keypress', '.forPrice', function(e){
			$(this).restrictKey(e, 'price');
		}).on('focusin', '.forPrice', function(){
	    	var c = this.value;
	      	this.value = c.replace(/,/g, '');
	      	var self = this;
	      	$(self).select();
	    }).on('focusout', '.forPrice', function(e){
	    	var c = this.value;
			this.value = $.addFigure(c);
		});
		
		// 日付　0から9 / - のみ入力し、不正値は""
		$(window).on('keypress', '.forDate', function(e){
			$(this).restrictKey(e,'date');
	    }).on('focusout', '.forDate', function(e){
	    	$.check_date(e, this);
	    });
